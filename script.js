...
function fileToDataUrl(file) {
  // 이미지가 아닌 파일(음악 등)은 그대로 base64로 읽는다.
  ...
  // 이미지는 캔버스로 한 번 리사이즈해서 저장한다. 게임 화면에서 어차피 작게(캔버스의
  // 몇 % 크기로) 표시되는데, 휴대폰 사진처럼 원본이 3000~4000px면 그대로 저장할 경우
  // localStorage 용량을 순식간에 다 써버린다. 투명도가 필요할 수 있으니 포맷(png/jpeg 등)은
  // 그대로 유지하고 "가로/세로 최대 크기"만 줄인다 - 화질 손실 없이 용량만 크게 줄어든다.
  const MAX_DIM = 1000;
    ...
    reader.onload = () => {
      ...
      img.onload = () => {
        ...
        if (width <= MAX_DIM && height <= MAX_DIM) {
          resolve(reader.result); // 이미 작으면 리사이즈할 필요 없이 원본 그대로
          return;
        }
      ...
      img.onerror = () => resolve(reader.result); // 디코딩 실패해도 원본이라도 쓸 수 있게
      img.src = reader.result;
    };
    reader.onerror = reject;
    reader.readAsDataURL(file);
  });
  // 이미지 디코딩이 어떤 이유로든 응답 없이 멈추는 극단적인 경우를 대비해, 일정 시간 넘으면
  // 리사이즈를 포기하고 그냥 원본 파일을 그대로 읽어서라도 업로드가 끝나게 한다.
  const timeoutFallback = new Promise((resolve) => {
    setTimeout(() => {
      const reader2 = new FileReader();
      reader2.onload = () => resolve(reader2.result);
      reader2.onerror = () => resolve(null);
      reader2.readAsDataURL(file);
    }, 4000);
  });
  return Promise.race([resizePromise, timeoutFallback]);
}
...
function createObject(type, overrides) {
  ...
  const spawnTime = overrides.spawnTime !== undefined ? overrides.spawnTime : 0; // 레이저 기본 타이밍 계산에 필요
  const base = {
    id: uid(),
    type,
    ...
    keyframes: [],
    zIndex: zCounter++,
    // 경고/발사 시각을 생성 시점(spawnTime) 기준 상대값으로 잡는다.
    // 이걸 절대시간 0~2로 고정해두면, 타임라인을 뒤로 옮긴 채로 레이저를 추가했을 때
    // "발사 구간이 이미 지나간 시간"이 되어 화면에 전혀 안 보이는 버그가 생긴다.
    laser: type === 'laser' ? { warnStart: spawnTime, fireStart: spawnTime + 1, fireEnd: spawnTime + 2, length: 60, thickness: 14 } : undefined,
...
function emptyBattle() {
  ...
  return {
    ...
    objects: [boss, player], dialogues: [], effects: [], attackCues: [],
  };
}
 ...
 */
function pickCurrentFrame(obj, battle, time, opts) {
  ...
  // 실시간 이벤트: 플레이어가 지금 막 공격했을 때 (플레이 모드 전용)
  ...
  // 타임라인 이벤트: 제작자가 타임라인에 직접 배치한 "공격 큐" (에디터 미리보기 + 실제 플레이 공통)
  const attackCue = findActiveAttackCue(battle, obj.id, time);
  if (attackCue && frames.attack && frames.attack.length) {
    const elapsed = Math.max(0, time - attackCue.time);
    const idx = Math.min(frames.attack.length - 1, Math.floor(elapsed * fps));
    return frames.attack[idx];
  }
...
function findActiveAttackCue(battle, objectId, time) {
  const cues = battle.attackCues;
  if (!cues) return null;
  for (const c of cues) {
    if (c.objectId === objectId && time >= c.time && time <= c.time + c.duration) return c;
  }
  return null;
}
...
function renderObjectNode(obj, battle, time, opts, outerContainer) {
  ...
  // ---- 피격/공격/사망 실시간 시각 효과 ----
  const frameSrc = pickCurrentFrame(obj, battle, time, opts);
  const usingDeathFrames = !!(opts.dying && opts.dying.objId === obj.id && obj.frames && obj.frames.death && obj.frames.death.length && frameSrc);
  const timelineAttackCue = findActiveAttackCue(battle, obj.id, time);
  const usingAttackFrames = !!(timelineAttackCue && obj.frames && obj.frames.attack && obj.frames.attack.length && frameSrc);
  ...
  // 타임라인 공격 큐: 실제 공격 프레임 이미지가 있으면 그걸로 대체되고,
  // 없으면(아직 이미지를 안 올렸으면) 부풀었다 가라앉는 펄스로라도 "공격 순간"을 보여준다.
  // 이건 에디터 미리보기(시간 스크럽)에서도 그대로 보여야 하므로 opts.now가 아니라 time 기준이다.
  if (timelineAttackCue && !usingAttackFrames) {
    const ct = clamp((time - timelineAttackCue.time) / timelineAttackCue.duration, 0, 1);
    const bump = Math.sin(ct * Math.PI); // 0 -> 1 -> 0 곡선
    extraScale *= 1 + bump * 0.3;
    filterStr += ` brightness(${1 + bump * 0.8})`;
  }
...
const editorState = {
  ...
  undoStack: [],
  redoStack: [],
  liveEditSession: false, // 속성 패널 입력창을 타이핑하는 동안 true - 히스토리/패널 재생성을 잠시 억제한다
};

function snapshotBattle() { return JSON.stringify(editorState.battle); }
function pushHistory() {
  if (editorState.liveEditSession) return; // 편집 세션 중엔 키 입력마다 기록하지 않고, 세션 시작 시 1번만 기록한다
...
function actionUpdateObject(id, patch, silent) {
  ...
  // 렌더링은 obj.x/y/scale/rotation/opacity가 아니라 항상 "현재 시간의 키프레임"을 보간해서 그린다.
  // 그래서 이 5개 값 중 하나라도 바뀌면, 지금 시간의 키프레임도 같이 갱신해줘야
  // 실제로 화면에 반영된다. 안 그러면 속성 패널에서 크기/회전/위치를 바꿔도
  // 데이터만 바뀌고 화면은 그대로인(=영영 안 바뀌는) 버그가 생긴다.
  if ('x' in patch || 'y' in patch || 'scale' in patch || 'rotation' in patch || 'opacity' in patch) {
    syncCurrentTransformKeyframe(o);
  }
  renderEditorAll();
}
function syncCurrentTransformKeyframe(o) {
  const t = editorState.currentTime;
  const idx = o.keyframes.findIndex((k) => Math.abs(k.time - t) < 0.01);
  if (idx >= 0) {
    Object.assign(o.keyframes[idx], { x: o.x, y: o.y, scale: o.scale, rotation: o.rotation, opacity: o.opacity });
  } else {
    o.keyframes.push(makeKeyframe({ time: t, x: o.x, y: o.y, scale: o.scale, rotation: o.rotation, opacity: o.opacity }));
  }
}
...
// ---- 공격 큐: 제작자가 타임라인의 특정 시점에 "여기서 공격해라"를 지정 ----
function actionAddAttackCue(objectId) {
  const obj = findObj(objectId);
  if (!obj) return;
  pushHistory();
  if (!editorState.battle.attackCues) editorState.battle.attackCues = [];
  const fps = obj.frameFps || 8;
  const framesLen = obj.frames && obj.frames.attack ? obj.frames.attack.length : 0;
  const duration = framesLen > 0 ? framesLen / fps : 0.4; // 공격 프레임이 있으면 그 길이만큼, 없으면 기본 0.4초
  editorState.battle.attackCues.push({ id: uid(), objectId, time: editorState.currentTime, duration });
  renderEditorAll();
}
function actionUpdateAttackCue(id, patch) {
  const cue = (editorState.battle.attackCues || []).find((c) => c.id === id);
  if (!cue) return;
  pushHistory();
  Object.assign(cue, patch);
  renderEditorAll();
}
function actionDeleteAttackCue(id) {
  pushHistory();
  editorState.battle.attackCues = (editorState.battle.attackCues || []).filter((c) => c.id !== id);
  renderEditorAll();
}
...
function actionSave() {
  try {
    battleRepository.save(editorState.battle);
    return true;
  } catch (e) {
    // 브라우저 저장공간(localStorage) 한도 초과 등으로 저장이 실패해도, 여기서 잡아서
    // 저장/재생 버튼 클릭 자체가 먹통이 되지 않도록 한다 (예전엔 여기서 예외가 그대로
    // 던져져서, 이 함수를 먼저 부르는 저장/재생 버튼이 둘 다 멈춰버렸다).
    console.error('저장 실패:', e);
    alert(
      '저장에 실패했습니다.\n\n' +
      '브라우저 저장공간(localStorage)이 가득 찼을 가능성이 높습니다. 특히 이미지·음악을 많이 올렸거나 전투 길이/오브젝트가 아주 많으면 브라우저 기본 저장 한도(보통 5~10MB)를 넘을 수 있습니다.\n\n' +
      '해결 방법: 상단의 "⬆ 가져오기/⬇ 내보내기" 중 "⬇ 내보내기"로 지금 상태를 파일로 먼저 백업하세요 (내보내기는 저장공간과 무관하게 항상 됩니다). 그 다음 이미지 용량을 줄이거나, 필요 없는 오브젝트/전투를 라이브러리에서 정리해보세요.'
    );
    return false;
  }
}
...
// 저장(localStorage)에 실패했더라도, 가져온 전투를 일단 편집은 할 수 있게 메모리에 잠깐 들고 있는 곳.
// (저장공간이 가득 찬 상태로 큰 파일을 가져와도 "불러오기 자체가 안 되는" 상황을 막기 위함)
let pendingImportBattle = null;

/**
 * JSON 파일에서 전투를 가져온다. 실패 원인을 정확히 구분해서 알려준다:
 * - JSON 자체가 깨졌다 -> "올바른 JSON 파일이 아닙니다"
 * - JSON은 멀쩡한데 저장 용량 초과 등으로 localStorage 저장만 실패 -> 그래도 편집 화면은 열어준다
 */
async function importBattleFile(file) {
  ...
  try {
    ...
    alert('전투 파일을 읽는 데 실패했습니다. 올바른 JSON 파일이 아닌 것 같습니다.\n\n(' + (e && e.message ? e.message : e) + ')');
    return;
  }
  if (!parsed || typeof parsed !== 'object' || !Array.isArray(parsed.objects)) {
    alert('JSON 파일은 맞지만, BATTLE MAKER 전투 데이터 형식이 아닌 것 같습니다 (objects 목록이 없습니다).');
    return;
  }
  parsed.id = uid();
  ...
  try {
    ...
    // JSON은 정상, 저장(localStorage 용량 등)만 실패한 경우 - 편집은 계속 가능하게 해준다.
    console.error('가져오기: 저장 실패, 메모리로만 열기', e);
    pendingImportBattle = parsed;
    alert(
      '전투 파일은 정상적으로 읽었지만, 브라우저 저장공간이 가득 차서 자동 저장은 되지 않았습니다.\n\n' +
      '일단 편집 화면은 열어드립니다. 이미지가 큰 오브젝트를 지우거나 다른 전투를 라이브러리에서 정리한 뒤 "저장"을 다시 눌러보세요.'
    );
    navigate(`#/edit/${parsed.id}`);
  }
}
...
function renderEditor(app, id) {
  let b;
  if (pendingImportBattle && pendingImportBattle.id === id) {
    b = pendingImportBattle;
    pendingImportBattle = null;
  } else {
    b = battleRepository.get(id);
  }
  ...
  shell.querySelector('#ed-save').onclick = () => {
    if (actionSave()) flashMessage('저장됨 ✓');
  };
  ...
  shell.querySelector('#ed-export').onclick = () => {
    // 내보내기는 localStorage와 무관하게 항상 동작해야 하므로 저장 성공 여부에 기대지 않는다.
...
function renderEditorAll() {
  ...
  renderToolPanel();
  renderTimeline();
  // 속성 패널 안의 입력창을 타이핑하는 도중(liveEditSession)엔 패널을 다시 그리지 않는다.
  // 안 그러면 지금 타이핑 중인 input이 새 DOM 노드로 교체되면서 포커스가 끊긴다.
  if (!editorState.liveEditSession) renderPropertyPanel();
}
...
function buildFrameAnimationSection(obj) {
  ...
  states.forEach(({ key, label }) => {
    ...
    // "공격" 상태에만: 타임라인의 특정 시점에 이 공격 애니메이션을 재생시키는 큐 관리 UI
    if (key === 'attack') {
      const cueWrap = el('div');
      cueWrap.style.cssText = 'margin-top:8px;padding-top:8px;border-top:1px dashed #2c2c33;';
      const addCueBtn = el('button', 'btn btn-primary btn-block btn-sm', `+ 여기(${editorState.currentTime.toFixed(2)}s)에 공격 추가`);
      addCueBtn.onclick = () => actionAddAttackCue(obj.id);
      cueWrap.appendChild(addCueBtn);

      const cues = (editorState.battle.attackCues || []).filter((c) => c.objectId === obj.id).sort((a, b) => a.time - b.time);
      if (cues.length) {
        const list = el('div');
        list.style.cssText = 'margin-top:6px;display:flex;flex-direction:column;gap:4px;';
        cues.forEach((c) => {
          const row = el('div');
          row.style.cssText = 'display:flex;align-items:center;gap:6px;background:#17171b;border-radius:4px;padding:4px 6px;font-size:11px;';
          const timeInput = el('input', 'field-input');
          timeInput.type = 'number'; timeInput.step = '0.1'; timeInput.value = c.time;
          timeInput.style.cssText = 'width:60px;padding:2px 4px;';
          timeInput.onchange = () => actionUpdateAttackCue(c.id, { time: Number(timeInput.value) });
          const durInput = el('input', 'field-input');
          durInput.type = 'number'; durInput.step = '0.1'; durInput.value = c.duration;
          durInput.style.cssText = 'width:56px;padding:2px 4px;';
          durInput.title = '지속시간(초)';
          durInput.onchange = () => actionUpdateAttackCue(c.id, { duration: Math.max(0.05, Number(durInput.value)) });
          const jumpBtn = el('button', '', '▶');
          jumpBtn.title = '이 시점으로 이동';
          jumpBtn.style.cssText = 'background:none;border:none;color:#9ad1ff;cursor:pointer;';
          jumpBtn.onclick = () => { editorState.currentTime = c.time; renderEditorAll(); };
          const rm = el('button', '', '✕');
          rm.style.cssText = 'background:none;border:none;color:#e0446f;cursor:pointer;margin-left:auto;';
          rm.onclick = () => actionDeleteAttackCue(c.id);
        ...
        cueWrap.appendChild(list);
      }
      cueWrap.appendChild(el('div', 'hint-text', '여기서 지정한 시간이 되면 에디터 미리보기와 실제 플레이 모두에서 이 오브젝트가 공격 프레임을 재생합니다 (프레임을 아직 안 올렸으면 대신 살짝 부풀어오르는 효과로 표시됩니다).'));
      box.appendChild(cueWrap);
    }
...
function renderPropertyPanel() {
  ...
  function textInput(value, onInput, type) {
    ...
    i.type = type || 'text';
    i.value = value;
    // 타이핑할 때마다(input) 즉시 반영하되, 패널 자체는 다시 그리지 않아서(liveEditSession)
    // 지금 포커스 중인 이 입력창이 사라지는 일이 없다. 대신 캔버스/타임라인은 매번 갱신되어
    // 크기·회전 같은 변화가 바로 눈에 보인다. 히스토리는 세션 시작 시 딱 1번만 기록하고,
    // 편집이 끝나면(change) 패널을 한 번 정식으로 다시 그려서 최종 값/포맷을 확정한다.
    i.oninput = () => {
      if (!editorState.liveEditSession) {
        pushHistory(); // 아직 liveEditSession=false라 여기서 정상적으로 undo 기록이 남는다
        editorState.liveEditSession = true;
      }
      onInput(type === 'number' ? Number(i.value) : i.value);
    };
    i.onchange = () => {
      editorState.liveEditSession = true; // 마지막 값 재적용 시 중복 기록 방지
      onInput(type === 'number' ? Number(i.value) : i.value);
      editorState.liveEditSession = false;
      renderPropertyPanel(); // 세션 종료 - 패널을 한 번만 정식으로 다시 그려 정리한다
    };
    i.onkeydown = (e) => { if (e.key === 'Enter') i.blur(); };
    i.addEventListener('blur', () => {
      // change 이벤트가 어떤 이유로든 안 터진 경우(예: 값이 원래와 같아짐)를 대비한 안전장치.
      // liveEditSession이 계속 true로 남아있으면 이후 모든 undo 기록/패널 갱신이 멈춰버린다.
      if (editorState.liveEditSession) {
        editorState.liveEditSession = false;
        renderPropertyPanel();
      }
...
function rebuildTimelineBody(tl, battle) {
  ...
  objectsByType.forEach((g) => {
    ...
    g.objects.forEach((o) => {
      ...
      (battle.attackCues || []).filter((c) => c.objectId === o.id).forEach((c) => {
        const cueBar = el('div', '');
        cueBar.style.cssText = `position:absolute;top:2px;height:6px;background:#ff6b3d;border-radius:2px;cursor:pointer;box-shadow:0 0 4px #ff6b3d99;`;
        cueBar.style.left = (c.time * editorState.pixelsPerSecond) + 'px';
        cueBar.style.width = Math.max(3, c.duration * editorState.pixelsPerSecond) + 'px';
        cueBar.title = `⚔ 공격 큐 t=${c.time.toFixed(2)}s~${(c.time + c.duration).toFixed(2)}s (Shift+클릭: 삭제)`;
        cueBar.onclick = (e) => {
          e.stopPropagation();
          if (e.shiftKey) { actionDeleteAttackCue(c.id); return; }
          // 키프레임 클릭과 동일하게, 토글하지 않고 항상 해당 오브젝트 패널을 열어 큐를 보여준다.
          editorState.currentTime = c.time;
          editorState.selectedObjectId = o.id;
          editorState.selectedDialogueId = null;
          editorState.showBattleSettings = false;
          renderEditorAll();
        };
        row.appendChild(cueBar);
      });
  ...
  if (query) {
    ...
    if (d) {
      ...
      if (decoded) {
        ...
        // (용량 초과로 실패해도 무시하고 지금 링크의 데이터로는 어차피 재생을 계속 진행한다)
        try {
          battleRepository.save(Object.assign({}, decoded, { updatedAt: new Date().toISOString() }));
        } catch (e) {
          console.error('공유 링크 자동 저장 실패(재생은 계속 진행):', e);
        }
      }
    }
  }
  if (!battle) battle = battleRepository.get(id);
  // 저장(localStorage)엔 실패했지만 에디터에 지금 열려있는 전투와 같은 id라면, 그 메모리 상의
  // 데이터로라도 재생한다 (용량 초과로 저장이 안 된 채로 바로 PLAY를 눌렀을 때를 위한 안전장치).
  if (!battle && editorState.battle && editorState.battle.id === id) battle = editorState.battle;
