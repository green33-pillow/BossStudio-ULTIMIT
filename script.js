...
function renderObjectNode(obj, battle, time, opts, outerContainer) {
  ...
  if (opts.now != null) {
    const hitUntil = opts.hitFlash && opts.hitFlash[obj.id];
    if (hitUntil && opts.now < hitUntil) {
      const remain = Math.max(0, hitUntil - opts.now) / 0.25; // 0~1
      filterStr += ` brightness(${1 + remain * 3.5}) saturate(${1 + remain * 2})`;
    }
    const pulseUntil = opts.attackPulse && opts.attackPulse[obj.id];
    if (pulseUntil && opts.now < pulseUntil) {
      const remain = Math.max(0, pulseUntil - opts.now) / 0.15;
      extraScale *= 1 + remain * 0.35;
    }
    if (opts.dying && opts.dying.objId === obj.id) {
  ...
  if (obj.shape === 'image' && obj.imageUrl) {
...
function attachDrag(node, obj, opts, outerContainer) {
  node.addEventListener('pointerdown', (e) => {
    e.stopPropagation();
    opts.onSelectObject && opts.onSelectObject(obj.id);
    ...
    const move = (ev) => {
      ...
      opts.onDragObject && opts.onDragObject(obj.id, nx, ny);
    };
...
function renderCanvas() {
  renderBattleCanvasInto(editorDom.canvas, editorState.battle, editorState.currentTime, {
    ...
    onSelectObject: (id) => { editorState.selectedObjectId = id; editorState.selectedDialogueId = null; renderToolPanel(); renderPropertyPanel(); renderTimelineSelectionOnly(); },
    onDragObject: (id, x, y) => actionMoveObjectTo(id, x, y),
  });
}
...
function rebuildTimelineBody(tl, battle) {
  ...
  objectsByType.forEach((g) => {
    ...
    g.objects.forEach((o) => {
      ...
      row.onclick = () => { editorState.selectedObjectId = o.id; editorState.selectedDialogueId = null; renderToolPanel(); renderPropertyPanel(); renderCanvas(); renderTimeline(); };
  ...
  battle.dialogues.forEach((d) => {
    ...
    row.onclick = () => { editorState.selectedDialogueId = d.id; editorState.selectedObjectId = null; renderPropertyPanel(); renderCanvas(); renderTimeline(); };
    labels.appendChild(row);
  });
    ...
    g.objects.forEach((o) => {
      ...
      o.keyframes.forEach((k) => {
        ...
        dia.onclick = (e) => {
          ...
          else { editorState.currentTime = k.time; renderCanvas(); renderTimeline(); }
        };
        row.appendChild(dia);
      });
...
function renderPlay(app, id, query) {
  ...
  function attack() {
    ...
    // 공격 펀치감 (플레이어 스프라이트 살짝 커짐)
    if (state.playerObjId) state.attackPulse[state.playerObjId] = t + 0.15;
    state.bossHp = Math.max(0, state.bossHp - battle.settings.playerAttackDamage);
    if (state.bossObjId) state.hitFlash[state.bossObjId] = t + 0.25;
  ...
  function dealDamageToPlayer(dmg) {
    ...
    if (state.playerObjId) state.hitFlash[state.playerObjId] = t + 0.25;
  ...
  // HP가 0이 되면 곧바로 오버레이를 띄우지 않고, 축소+페이드+충격파 연출을 먼저 재생한다.
  function triggerDeath(which) {
    ...
    state.dying = { which, objId, start: performance.now() / 1000 };
    battle.effects.push({ id: 'rt-' + uid(), time: state.currentTime, duration: DEATH_ANIM_DURATION, type: 'shockwave', runtime: true });
  ...
  function loop(now) {
    ...
    const dyingOpts = state.dying ? { objId: state.dying.objId, start: state.dying.start, duration: DEATH_ANIM_DURATION } : null;
    ...
    if (state.status === 'dying' && nowSec - state.dying.start >= DEATH_ANIM_DURATION) {
